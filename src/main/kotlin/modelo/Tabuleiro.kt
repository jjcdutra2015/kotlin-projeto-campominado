...
class Tabuleiro(val qtdeLinhas: Int, val qtdeColunas: Int, private val qtdeMinas: Int) {
    ...
    private fun sortearMinas() {
        ...
        var linhaSorteada = -1
        var colunaSoreteada = -1
        var qtdeMinasAtual = 0

        while (qtdeMinasAtual < this.qtdeMinas) {
            linhaSorteada = gerador.nextInt(qtdeLinhas)
            colunaSoreteada = gerador.nextInt(qtdeColunas)

            val campoSorteado = campos[linhaSorteada][colunaSoreteada]
    ...
    fun forEachCampo(campo: (Campo) -> Unit) {
        campos.forEach { linha -> linha.forEach { campo } }
    }
